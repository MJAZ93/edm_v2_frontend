...
export default function ProvinciasScreen() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const { data } = await api.privateProvincesGet(
        authHeader,
        page,
        pageSize,
        orderBy,
  ...
  async function handleDelete(id: string) {
    const ok = confirm('Eliminar a província selecionada?'); if (!ok) return
    try {
      ...
      const status = err?.response?.status
      if (status === 401 || isUnauthorizedBody(err?.response?.data)) { logout('Sessão expirada. Inicie sessão novamente.'); return }
      alert(!status ? 'Sem ligação ao servidor.' : status >= 500 ? 'Erro do servidor ao eliminar província.' : 'Falha ao eliminar província.')
    }
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <Button onClick={() => setShowCreate(true)}>Nova província</Button>
        </div>
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {ui.loading ? (
                ...
                <tr><td colSpan={2} style={{ padding: 16, color: '#6b7280' }}>Sem províncias para mostrar.</td></tr>
              ) : (
                items.map((p) => (
                  <tr key={p.id}>
                    <td style={{ padding: '10px 8px', borderBottom: '1px solid #f3f4f6' }}>{p.name}</td>
                    <td style={{ padding: '10px 8px', borderBottom: '1px solid #f3f4f6', display: 'flex', gap: 8 }}>
                      ...
                      <Button variant="danger" onClick={() => p.id && handleDelete(p.id)}>Eliminar</Button>
                    </td>
                  </tr>
                ))
              )}
      ...
      {(showCreate || editing) && (
        <div role="dialog" aria-modal="true" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.25)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={() => { if (submitting) return; setShowCreate(false); setEditing(null); setSubmitError(null) }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: '#fff', borderRadius: 12, padding: 20, width: '100%', maxWidth: 420 }}>
            <h3 style={{ marginTop: 0 }}>{showCreate ? 'Criar província' : 'Editar província'}</h3>
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); if (!name.trim()) return; onSubmit(name.trim()) }} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <label style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Nome da província" style={{ padding: 12, borderRadius: 8, border: '1px solid #d1d5db' }} />
      </label>
      <div style={{ display: 'flex', gap: 8 }}>
